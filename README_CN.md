...
1. 从 [Releases](https://github.com/ZhaoBuyan/MuSync/releases) 下载 **MuSync.exe**（完整版，推荐——免安装单文件，双击即用）
2. 首次启动登录 Steam（支持手机令牌 / 邮箱验证码）。若遇到「异常登录」提示，按登录窗口中的指引操作：手机 Steam App → 选择「Steam 客户端」→ 确认实际所在地
3. 打开音乐播放器——完成，好友已经能看到你在听什么
4. （可选）想让好友看到你在用什么程序：设置 → 同步 → 打开「同步非游戏应用」
...
- 你正在听的歌，本来只有自己知道；MuSync 把它变成好友列表里的一行状态
- 不用装客户端插件、不用开着 Steam 客户端——登录一次，之后就自动同步
- 听歌之外的程序状态也能同步（在 VS Code 写代码、玩非 Steam 启动的游戏），字句 / 样式 / 颜色全部由你定义
